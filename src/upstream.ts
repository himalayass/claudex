...
export function isChatgptCodexOrigin(origin: URL): boolean {
  return origin.hostname === "chatgpt.com" && normalizeBasePath(origin.pathname).startsWith("/backend-api/codex");
}

export function buildUpstreamUrl(upstreamOrigin: URL, requestPath: string): URL {
...
export function rewriteRequestPath(
  upstreamOrigin: URL,
  requestPath: string,
  wireApi: "messages" | "responses"
): string {
  const incoming = new URL(requestPath, "http://localhost");

  if (wireApi === "responses" && incoming.pathname === "/v1/messages") {
    if (isChatgptCodexOrigin(upstreamOrigin)) {
      incoming.pathname = "/responses";
    } else {
      incoming.pathname = "/v1/responses";
    }
  }
...
export function mergeRequestHeaders(req: http.IncomingMessage, authState: AuthState): Record<string, string> {
  const skip = new Set(["host", "content-length", "authorization", "accept-encoding"]);
